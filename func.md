...
- Сложность задачи - число от 1 до 10.
    ...
    - Примерное соответствие уровня задачи этапам ВСОШ:
        - 1 - элементарная A+B
        - 2 - ниже уровня региона;
        - 3 - A регион;
        - 4 - A-B регион;
        - 5 - B регион;
        - 6 - A всерос / B-C регион;
        - 7 - C регион;
        - 8 - B всерос / C-D регион;
        - 9 - D регион;
        - 10 - C всерос и выше.
...
- Дуэлянтам выдаётся одна задача, сложность которой зависит от рейтинга дуэлянтов.
...
- Если время длительности дуэли превысило максимальную длительность (30 минут), объявляется ничья.
...
- Если не все тесты имеют вердикт Зачтено, решение Не зачтено.
...
Дуэль можно конфигурировать. В таком случае дуэль автоматически становится нерейтинговой.
Далее описаны характеристики, доступные для конфигурирования.
...
- Можно выбирать темы, которые будут встречаться в задачах. В таком случае каждая задача будет содержать каждую из указанных тем.

### Поиск дуэли

- Рейтинговая дуэль - поиск рейтинговой дуэли среди пользователей, также находящихся в поиске рейтинговой дуэли.
- Вызов на дуэль - выбрать конфигурацию дуэли, дуэлянта и отправить ему вызов на дуэль. Для старта дуэли дуэлянт должен принять вызов.
...
- Менеджер имеет возможность создать дуэль со своей конфигурацией и выбрать двух дуэлянтов среди пользователей группы.
- Все дуэли в группе являются нерейтинговыми.

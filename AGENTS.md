...
- The root `CoDuels` repository manually releases the revisions pinned by `master` through `release-images.yml`. Given a semantic version, it runs the existing component Ansible build playbooks, pushes the versioned images to Docker Hub, and creates the matching Git tag and GitHub Release. It does not deploy services. Advancing a root submodule pointer alone does not trigger a release or deploy anything.
...
- Root releases require `DOCKER_PASSWORD`. The Frontend image receives `VITE_BASE_URL` at container startup rather than build time.
